...
export class SobreComponent {
  ...
  AlteraSobre(t: string, c: string, e: Event) {
    if (t != '' || c != '') {
      this.m = false;
      this.http.put<any>(`http://localhost:3000/sobre/${this.Sobre[0].id}`, { titulo: t, conteudo: c }, { responseType: 'json' }).subscribe(data => {
        this.postId = data.id;
  ...
  functionLogin(u: string, p: string, e: Event) {
    e.preventDefault();
    this.http.post<any>(`http://localhost:3000/usuario1`, { username: u, password: p }, { responseType: 'json' }).subscribe(data => {
